...
bool ratmaze(int n, int m, int row, int col) {
    ...
    if (maze[row][col+1] == 'O' && result[row][col+1]!=1) {    //move right
		//cout << "moving right\n" ;
		result[row][col+1] = 1 ;
	...
	if (maze[row+1][col] == 'O' && result[row+1][col]!=1) { 		//move down
		//cout << "moving down\n" ;
		result[row+1][col] = 1 ;
	...
	if (maze[row][col-1] == 'O'  && col>0  && result[row][col-1]!=1) {    //move left
		//	cout << "moving left\n" ;
			result[row][col-1] = 1 ;
		    if(ratmaze(n, m, row, col-1)) return true;
		    result[row][col-1] = 0;
