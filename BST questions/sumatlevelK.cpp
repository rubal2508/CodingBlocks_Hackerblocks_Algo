...
public:
    int data;
    vector <generic_node*> children;
    int no_child;

    generic_node(int d,int n){
        data = d;
        children.resize(n);
    }
};

generic_node* build_generictree(){
    int data,size;
    cin>>data>>size;

    generic_node* root = new generic_node(data,size);
    loop(i,size){
        root->children[i] = build_generictree();
    }
    return root;
}


void sumatK(generic_node* root, int k){
    static int sum = 0;
    if(root==NULL) return;
    if(k==0) {
        sum += root->data;
    ...
    loop(i,root->no_child){
        sumatK(root->children[i],k-1);
    }
...
int main(){
    ...
    generic_node* root = build_generictree();
